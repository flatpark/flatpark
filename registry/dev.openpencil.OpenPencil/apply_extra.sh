...
# package is a plain FHS tree holding a single Tauri binary at usr/bin/OpenPencil
# plus its icons and .desktop file; there is no Tauri resource directory beside
# it. The whole usr tree is still staged as-is at /app/extra/usr so that any
# resource directory a future release adds keeps resolving relative to the
# executable, and the wrapper launches it through /app/extra/bin/openpencil.
#
...
[ -f openpencil.deb ] || { echo "missing extra-data: openpencil.deb" >&2; exit 1; }
...
bsdtar -xOf openpencil.deb 'data.tar*' | bsdtar --no-same-owner -xf - -C stage
...
# link named openpencil rather than one named after the artifact.
mkdir -p bin
ln -sf "../usr/bin/$exec_name" bin/openpencil
