...
# package is a plain FHS tree: the Tauri binary at usr/bin/note-gen plus its
# Tauri resource directory at usr/lib/NoteGen (icons and the OCR resource dir).
# Tauri resolves that resource directory relative to the executable
# (<exe>/../lib/NoteGen), so the whole usr tree is staged as-is at
# /app/extra/usr; the wrapper launches it through /app/extra/bin/note-gen.
#
...
[ -f note-gen.deb ] || { echo "missing extra-data: note-gen.deb" >&2; exit 1; }
...
bsdtar -xOf note-gen.deb 'data.tar*' | bsdtar --no-same-owner -xf - -C stage
...
# link named note-gen rather than one named after the artifact. The app's own
# resource lookup goes through /proc/self/exe, which resolves back to
# usr/bin/<launcher>, so ../lib/NoteGen still resolves.
mkdir -p bin
ln -sf "../usr/bin/$exec_name" bin/note-gen
