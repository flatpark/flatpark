...
/* This library is already mapped into the zapfast process by the time the
 ...
 * value there is rejected with EPERM, which would break opening a downloaded
 * attachment. The tray backend inside zapfast still sees the fixed PID. */
...
pid_t getpid(void)
{
    const char *value = getenv("ZAPFAST_FAKE_PID");
    char *end = NULL;
    long parsed;
